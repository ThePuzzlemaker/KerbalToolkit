...
impl GridBuilder {
    ...
    pub fn show(self, ui: &mut Ui, grid: impl FnOnce(Grid)) -> Response {
        ...
        let pure_cells = self.as_real_cells(allocated_space, ui.style().spacing.item_spacing);
    ...
    fn as_real_cells(&self, whole_rect: Rect, def_spacing: Vec2) -> Vec<PureCell> {
        ...
        for (row_index, row) in self.units.iter().enumerate() {
            ...
            for (cell_index, cell) in row.cells.iter().enumerate() {
                ...
                match &row.cells[cell_index].group {
                    Option::Some(grid) => {
                        cells_final.extend(grid.as_real_cells(rect, def_spacing));
                    }
                    Option::None => {
